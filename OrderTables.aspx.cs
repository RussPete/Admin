...
public partial class OrderTables : System.Web.UI.Page
{
	...
	private void Page_PreRender(object sender, System.EventArgs e)
	{
        ...
        NewHtml = Misc.ToString(EditValue("xxxx", 0, "New Table"));
	}

    private void LoadValues()
    {
        string Sql = "Select TableRno, Description From JobTables Where JobRno = 0 Order By Seq";
        try
        {
            DB db = new DB();
            ...
            {
                int TableRno = DB.Int32(dr["TableRno"]);
                string Value = DB.Str(dr["Description"]);
                ulItems.Controls.Add(EditValue(i.ToString(), TableRno, Value));
                i++;
            }
            db.Close();
        }
		...
		try
		{
			...
			for (int i = 0; i < cItems; i++)
			{
                ...
                if (Rno == 0)
                {
                    if (!fDelete)
                    {
                        Sql = string.Format("Insert Into JobTables (JobRno, Description, Seq, CreatedDtTm, CreatedUser) Values (0, {0}, {1}, GetDate(), {2})",
                            DB.PutStr(NewValue),
                            Seq,
                            DB.PutStr(g.User));
                        db.Exec(Sql);
                ...
                else
				{
                    if (!fDelete)
                    {
                        Sql = string.Format("Update JobTables Set Seq = {1}, UpdatedDtTm = GetDate(), UpdatedUser = {2} Where TableRno = {0}",
                            Rno,
                            Seq,
                            DB.PutStr(g.User));
                        db.Exec(Sql);
                        ...
                        if (NewValue != Value)
                        {
                            ...
                            Sql = string.Format("Update JobTables Set Description = {1} Where TableRno = {0}",
                                Rno,
                                DB.PutStr(NewValue));
                            db.Exec(Sql);
                        }
                    ...
                    {
                        Sql = string.Format("Delete From JobTables Where TableRno = {0}", Rno);
                        db.Exec(Sql);
                    }
				}
			}
