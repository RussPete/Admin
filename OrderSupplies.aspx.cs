...
public partial class OrderSupplies : System.Web.UI.Page
{
	...
	private void Page_PreRender(object sender, System.EventArgs e)
	{
        ...
        NewHtml = Misc.ToString(EditValue("xxxx", 0, "New Supply"));
	}

    private void LoadValues()
    {
        string Sql = "Select SupplyRno, SupplyItem From mcJobSupplies Where JobRno = 0 Order By SupplySeq";
        try
        {
            DB db = new DB();
            ...
            {
                int SupplyRno = DB.Int32(dr["SupplyRno"]);
                string Value = DB.Str(dr["SupplyItem"]);

                HtmlGenericControl ulItems = (i < (cRows + 1) / 2 ? ulItems1 : ulItems2);
                ulItems.Controls.Add(EditValue(i.ToString(), SupplyRno, Value));
                i++;
            }
            db.Close();
        }
