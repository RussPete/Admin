...
public partial class Demo_InitData : System.Web.UI.Page
{
    ...
    protected void ImportData()
    {
        ...
        */
        ...
        string Sql = "select MenuItemRno, Category, MenuItem from mcJobMenuItems";
        DataTable dtMenuItems = null;
        try
        {
            dtMenuItems = db.DataTable(Sql);
        }
        catch (Exception Ex)
        {
        ...
        for (int iCustomer = 0; iCustomer < NumCustomers; iCustomer++)
        {
            ...
            for (int iContact = 0; iContact < NumContacts; iContact++)
            {
                ...
                for (int iJob = 0; iJob < NumJobs; iJob++)
                {
                    ...
                    if (dtMenuItems != null)
                    {
                        int NumFoods = Rnd.Next(MaxFood) + 1;
                        for (int iFood = 0; iFood < NumFoods; iFood++)
                        {
                            int iMenuItem = Rnd.Next(dtMenuItems.Rows.Count);
                            Response.Write(iMenuItem.ToString() + "<br/>");
                            DataRow dr = dtMenuItems.Rows[iMenuItem];
                            string Category = DB.Str(dr["Category"]);
                            string MenuItem = DB.Str(dr["MenuItem"]);
                            int MenuItemRno = DB.Int32(dr["MenuItemRno"]);
                            InsertJobFood(JobRno, iFood + 1, Category, MenuItem, MenuItemRno);
                        }
                    }
                }
            }
    ...
    {
        string Sql = string.Format("delete from mcJobFood");
        try
        {
            db.Exec(Sql);

            Sql = string.Format("delete from mcJobs");
            db.Exec(Sql);
        }
        catch (Exception Ex)
        {
    ...
    protected int InsertJobFood(int JobRno, int FoodSeq, string Category, string MenuItem, int MenuItemRno)
    {
        int Rno = 0;

        string Sql = string.Format(
            "insert into mcJobFood (JobRno, FoodSeq, Category, MenuItem, MenuItemRno, CreatedDtTm, CreatedUser) values ({0}, {1}, {2}, {3}, {4}, GetDate(), 'Sys'); select SCOPE_IDENTITY()", 
            JobRno, FoodSeq, DB.Put(Category, 50), DB.Put(MenuItem, 50), MenuItemRno);
        try
        {
            Rno = db.SqlNum(Sql);
        }
