...
public partial class OrderServices : System.Web.UI.Page
{
	...
	private void Page_PreRender(object sender, System.EventArgs e)
	{
        ...
        NewHtml = Misc.ToString(EditValue("xxxx", 0, "New Service"));
	}

    private void LoadValues()
    {
        string Sql = "Select ServiceRno, ServiceItem From mcJobServices Where JobRno = 0 Order By ServiceSeq";
        try
        {
            DB db = new DB();
            ...
            {
                int ServiceRno = DB.Int32(dr["ServiceRno"]);
                string Value = DB.Str(dr["ServiceItem"]);

                HtmlGenericControl ulItems = (i < (cRows + 1) / 2 ? ulItems1 : ulItems2);
                ulItems.Controls.Add(EditValue(i.ToString(), ServiceRno, Value));
                i++;
            }
            db.Close();
        }
		...
		try
		{
			...
			for (int i = 0; i < cItems; i++)
			{
                ...
                if (Rno == 0)
                {
                    if (!fDelete)
                    {
                        Sql = string.Format("Insert Into mcJobServices (JobRno, ServiceItem, ServiceSeq, CreatedDtTm, CreatedUser) Values (0, {0}, {1}, GetDate(), {2})",
                            DB.PutStr(NewValue),
                            Seq,
                            DB.PutStr(g.User));
                        db.Exec(Sql);
                ...
                else
				{
                    if (!fDelete)
                    {
                        Sql = string.Format("Update mcJobServices Set ServiceSeq = {1}, UpdatedDtTm = GetDate(), UpdatedUser = {2} Where ServiceRno = {0}",
                            Rno,
                            Seq,
                            DB.PutStr(g.User));
                        db.Exec(Sql);
                        ...
                        if (NewValue != Value)
                        {
                            ...
                            Sql = string.Format("Update mcJobServices Set ServiceItem = {1} Where ServiceRno = {0}",
                                Rno,
                                DB.PutStr(NewValue));
                            db.Exec(Sql);
                        }
                    ...
                    {
                        Sql = string.Format("Delete From mcJobServices Where ServiceRno = {0}", Rno);
                        db.Exec(Sql);
                    }
				}
			}
